...
let proceedFromHomeButtonOnly = sessionStorage.getItem(
  "proceedFromHomeButtonOnly"
);
...
if (currentUser) {
  ...
  if (proceedFromHomeButtonOnly == "false") {
    swal(
      "Before starting face verification, fill transaction details at home page.",
  ...
  return Promise.all(
    labels.map(async (label) => {
      ...
      if (imgURL == "null") {
        swal(
          "Update Profile!",
          "To start face verification, Please upload the profile picture first in the profile section.\n\nPressing 'OK' will redirect you to profile section.",
          "warning"
        ).then(function (reply) {
...
async function matchFace() {
  ...
  setTimeout(() => {
    ...
    if (showStartBtn) startBtn.classList.replace("hide", "unhide");
      ...
      swal("Something went wrong!", "Please refresh the page.", "error").then(
        function (reply) {
  ...
  recognizing = async () => {
    ...
    if (!resizedDetections[0])
      swal(
        ...
        "Please take the shot near better light source.\nOr, Try removing the spectacles/glasses or mask.\nOr, Try updating the profile picture.",
        "warning"
      );
...
function makePayment() {
  ...
  if (faceLabel == "known" && faceScore <= 0.45) {
    faceVerified = true;
    swal(
      `${currentUser.fullname}, you are now verified!`,
      "Press the pay button to make payment.",
      "success"
    );
    ...
    swal(
      "Verification Failed!",
      "Face was not matched with the profile",
      "error"
    ).then(function () {
